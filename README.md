...
The subagent-file split adds seven new fields:
...
- `subagent_dispatches` — an array with one record per `agent-*.jsonl` file (not just the session-wide sums above): `agent_id`, `input_tokens`, `output_tokens`, `cache_read_tokens`, `cache_write_tokens`, `agent_type`, `description`, `tool_use_id`, `spawn_depth`, `model`. The four label fields (`agent_type`/`description`/`tool_use_id`/`spawn_depth`) come from the sibling `agent-<id>.meta.json` sidecar and are `null` when that file is missing or unparseable — never a dropped record. `model` comes from `message.model` in the transcript itself (not the sidecar) and is `null` in two cases: no real model was ever seen on the file (e.g. its only usage-bearing lines carried Claude Code's `"<synthetic>"` error-line sentinel, which is filtered before consideration and never reported as a model), or the file genuinely mixed two or more different real models across its lines — reporting either model in that case would silently misattribute the other model's tokens, so `null` is preferred over a plausible wrong name.
...
- **subagent tokens — two sources, may legitimately diverge**:
  ...
  - `subagent_input_tokens` / `subagent_output_tokens` / `subagent_cache_read_tokens` / `subagent_cache_write_tokens` / `subagent_agent_file_count` / `subagent_usage_total_tokens` / `subagent_dispatches`: aggregated directly from the `subagents/agent-*.jsonl` transcripts, the first six as session-wide sums and `subagent_dispatches` as the per-file records those sums are built from. **Authoritative for cost extrapolation** — it's the only source that separates input from output from cache, which are priced very differently. Requires the transcript to still be in its original directory (see "Session files"); if the `subagents/` dir isn't found, these fields are 0 (empty array for `subagent_dispatches`) and a note is printed to stderr — not an error. Per-record `model` may be `null` (see "Output formats" above) — treat a `null` model as "unknown," never as "attribute this record's tokens to whatever model the rest of the dispatch used."
